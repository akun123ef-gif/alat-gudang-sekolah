<?php
	...
	include 'db.php';
	trace_event("OPEN PAGE data_request.php username=$username");
?>
<!DOCTYPE html>
<html>
<head>
				...
				<table class="table table-bordered table-super-condensed">
					...
					<tbody>
						<?php
							...
							$sql = "SELECT * FROM tbl_request WHERE peminjam='$username' ORDER BY id DESC";
							$query = db_query($conn,$sql);

							trace_event("RUN QUERY tbl_request peminjam=$username");

							if(mysqli_num_rows($query) == 0){
								trace_event("RESULT EMPTY tbl_request peminjam=$username");
								echo "<tr><td colspan='6'>belum ada data!</td></tr>";
							}else{
								trace_event("RESULT FOUND tbl_request peminjam=$username");
								$no = 1;
